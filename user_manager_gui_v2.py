"""
User Manager GUI V2 - Secure interface with audit logging

Features:
- No clear text password display (always show asterisks)
- Role-based permissions (users can only change their own password)
...
class UserManagerGUI:
    """GUI for managing users with role-based permissions and security."""
    ...
    def _build_ui(self):
        ...
        if self.is_admin:
            ttk.Button(toolbar, text="📋 View Audit Log", command=self._view_audit_log, style="Modern.TButton").pack(side="left", padx=(0, 5))
        ...
        columns = ("Full Name", "Username", "Role", "Created By", "Updated By", "Last Login")
        ...
        self.tree.heading("Updated By", text="Updated By")
        self.tree.heading("Last Login", text="Last Login")
        
        self.tree.column("Full Name", width=180)
        self.tree.column("Username", width=120)
        self.tree.column("Role", width=80)
        self.tree.column("Created By", width=120)
        self.tree.column("Updated By", width=120)
    ...
    def _change_password(self):
        ...
        # Get user info (but don't show password)
        user = self.user_manager.find_by_username(username)
        if not user:
            messagebox.showerror("Error", "User not found")
            return
        
        # Show password dialog (no current password display)
...
class PasswordDialog:
    """Dialog for changing password - NEVER shows current password."""
    ...
    def _build_ui(self):
        ...
        # Info label
        info_label = ttk.Label(frame, text=f"Setting new password for: {self.username}",
                              font=("Segoe UI", 10, "bold"))
        info_label.grid(row=0, column=0, sticky="w", pady=(0, 20))
        
        # New Password
        ttk.Label(frame, text="New Password:").grid(row=1, column=0, sticky="w", pady=(0, 5))
        ...
        # Confirm Password
        ttk.Label(frame, text="Confirm Password:").grid(row=3, column=0, sticky="w", pady=(0, 5))
        ...
        # Buttons
        button_frame = ttk.Frame(frame)
        button_frame.grid(row=6, column=0, sticky="e", pady=(10, 0))
...
class AuditLogViewer:
    """Viewer for user audit and activity logs (Admin only)."""
    ...
    def _build_ui(self):
        ...
        notebook.add(audit_frame, text="Audit Log (User Management)")
        
        # Toolbar for audit log
        audit_toolbar = ttk.Frame(audit_frame)
        audit_toolbar.pack(fill="x", pady=(0, 10))
        
        ttk.Label(audit_toolbar, text="Filter by target:").pack(side="left", padx=(0, 5))
        ...
        notebook.add(activity_frame, text="Activity Log (User Actions)")
        
        # Toolbar for activity log
        activity_toolbar = ttk.Frame(activity_frame)
        activity_toolbar.pack(fill="x", pady=(0, 10))
        
        ttk.Label(activity_toolbar, text="Filter by user:").pack(side="left", padx=(0, 5))
        ...
        activity_columns = ("Timestamp", "User", "Activity", "Description", "AP ID")
        self.activity_tree = ttk.Treeview(activity_frame, columns=activity_columns, show="headings",
                                         yscrollcommand=activity_scroll.set)
        activity_scroll.config(command=self.activity_tree.yview)
        
        for col in activity_columns:
            self.activity_tree.heading(col, text=col)
        
        self.activity_tree.column("Timestamp", width=180)
        self.activity_tree.column("User", width=120)
        self.activity_tree.column("Activity", width=150)
        self.activity_tree.column("Description", width=400)
        self.activity_tree.column("AP ID", width=120)
    ...
    def _refresh_activity_log(self):
        ...
        # Get filtered logs
        username = self.activity_filter_var.get().strip() or None
        logs = self.user_manager.get_user_activity_log(username=username, limit=500)
